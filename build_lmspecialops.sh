...
ln -s /usr/lib/python3.6/site-packages/tensorflow/libtensorflow_framework.so.1 /usr/lib/python3.6/site-packages/tensorflow/libtensorflow_framework.so
...
LD_LIBRARY_PATH=/usr/local/$CUDA_DIR_NAME/targets/x86_64-linux/lib/stubs/ cmake -DCMAKE_BUILD_TYPE=Release  -DCMAKE_C_COMPILER=/usr/bin/gcc-4.9 -DCMAKE_CXX_COMPILER=/usr/bin/g++-4.9 -DTENSORFLOW_INCLUDE_DIR=/usr/lib/python3.6/site-packages/tensorflow/include -DTENSORFLOW_FRAMEWORK_LIB=/usr/lib/python3.6/site-packages/tensorflow/libtensorflow_framework.so -DCUDA_ARCH_LIST="5.2" -DCMAKE_LINK_WHAT_YOU_USE=1 -UCMAKE_CXX_STANDARD -DCMAKE_CXX_FLAGS="-mno-avx512f -D_GLIBCXX_USE_CXX11_ABI=0" -DPYTHON_EXECUTABLE=/usr/bin/python3.6 ..
if [ "$?" != "0" ]
then
	exit 1
fi
